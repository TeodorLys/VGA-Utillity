...
void Thread_Rendering(sf::RenderWindow *window) {
	...
	while (window->isOpen()) {
		if (Shared_bool::movie_is_Playing && Main_Runtime_Check::border && !windowClosed) {
			...
			window->clear();  //Clears the color of the screen
...
void Main_Window_PollEvent(sf::Thread &render) {
 while (Shared_sf::window.pollEvent(Shared_sf::event)) {
		...
		case sf::Event::Closed:
			...
			render.terminate();   //Detaches the thread so it can safely exit		
...
int main(int argc, char* argv[]) {
	...
	sf::Thread render(Thread_Rendering, &Shared_sf::window);   //Sets up the thread
	...
	while (Shared_sf::window.isOpen()) {
		...
		//Changes the movie object, to multi or single. If it should ex. pause/play two movies or one
		...
		Main_Runtime_Check::When_Movie_is_Playing(obj.movie, obj.md, &cv, &m, &pause, &moving_Second_Movie);
