...
Buttons::Buttons(sf::RenderWindow *w, sf::Mouse *m, sf::Font *f, bool *fo) :
	window(w),
	mouse(m),
	font(f),
	focus(fo)
...
void Buttons::setText(string str) {
	...
	title.setFont(*font);   //Sets the font 
...
bool Buttons::Hover(sf::Color c, float size) {
	if (b.getGlobalBounds().contains(sf::Vector2f(mouse->getPosition(*window))) && focus) {
...
void Buttons::ScreenScaleing(sf::Vector2f pos, float sizeX, float sizeY, float extra) {
	newScaleX = (sizeX - 1) + window->getSize().x / 720;
	newScaleY = (sizeX - 1) + window->getSize().x / 720;
